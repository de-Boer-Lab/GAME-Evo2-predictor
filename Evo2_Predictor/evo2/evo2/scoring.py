...
#This function could also eventually be extended to return track predictions
...
##Function added by Ishika to make track predictions
def logits_to_logprobs_track(
        logits: torch.Tensor,
        input_ids: torch.Tensor,
        scale: str,
) -> torch.Tensor:
    ...
    if scale == 'log':
        #This converts logits to log probabilites
        softmax_probs = torch.log_softmax(logits, dim=-1)

    else:
        #This converts to probabilities
        softmax_probs = torch.softmax(logits, dim=-1)

    #Last logit is predicting a token that doesn't exist in the sequence, crop it
    softmax_probs = softmax_probs[:, :-1]
    input_ids = input_ids[:, 1:]
    assert softmax_probs.shape[1] == input_ids.shape[1]
    logprobs = torch.gather(
        softmax_probs,       # Gather likelihoods...
        2,                      # along the vocab dimension...
        input_ids.unsqueeze(-1) # using the token ids to index.
    ).squeeze(-1)
...
def _score_sequences_track(
    ...
    """Helper function to score a list of sequences for a track request"""
    ...
    logprobs = logits_to_logprobs_track(logits, input_ids, scale=scale)
    logprobs = logprobs.float().cpu().numpy()
    return [
        logprobs[idx][:seq_lengths[idx]]  # (seq_len,)
        for idx in range(len(seq_lengths))
    ]

def score_sequences_track(
        seqs: List[str],
        scale: str,
        model: StripedHyena,
        tokenizer: object,
    ...
    """Returns the per-position log-likelihood of each observed base for each
    sequence (not the full vocab distribution).

    For each sequence, gathers the model's predicted probability of the base that
    is actually present at each position. If `scale='log'`, values are
    log-probabilities; otherwise they are linear probabilities.

    Returns a list of arrays, one per input sequence, each of shape (seq_len,).
    """
    ...
    for i in tqdm(range(0, len(seqs), batch_size)):
        batch_seqs = seqs[i:i + batch_size]
        batch_scores = _score_sequences_track(
            batch_seqs,
            scale,
            model,
            tokenizer,
