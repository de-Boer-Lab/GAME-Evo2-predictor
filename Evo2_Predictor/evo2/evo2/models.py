...
from evo2.scoring import score_sequences, score_sequences_rc, score_sequences_track
...
class Evo2:
    ...
    def score_sequences_track(
        ...
        with torch.no_grad():
            try:
                scores = scoring_func(seqs, scale)
            except Exception as e:
                raise RuntimeError(f"Error during track scoring: {str(e)}") from e
